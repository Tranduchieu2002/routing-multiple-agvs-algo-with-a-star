...
def analyze_algorithms():
    input_sizes = [10, 100, 1000, 2000, 5000]
    a_star_time_avg = []
    a_star_steps_avg = []
    a_star_memory_avg = []
    ...
    for size in input_sizes:
        ...
        # Calculate averages
        a_star_time_avg.append(sum(a_star_time) / len(a_star_time))
        a_star_steps_avg.append(sum(a_star_steps) / len(a_star_steps))
        a_star_memory_avg.append(sum(a_star_memory) / len(a_star_memory))

        dijkstra_time_avg.append(sum(dijkstra_time) / len(dijkstra_time))
        dijkstra_steps_avg.append(sum(dijkstra_steps) / len(dijkstra_steps))
        dijkstra_memory_avg.append(sum(dijkstra_memory) / len(dijkstra_memory))
